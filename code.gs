...
/* ============================================================================
 *  PAGE 10 — JADWAL DINAS (matriks, READ-ONLY)
 *  Port dari web app "Daftar Dinas" (Tab 1). Hanya jalur BACA; semua fungsi
 *  tulis/admin/form dibuang. Spreadsheet = PD_SS_ID (sama dgn Page7).
 *  Fungsi client mengembalikan JSON string (konvensi CLAUDE §4).
 * ==========================================================================*/
var JD_LOOKUP_SHEET = 'LookUp';
/* Kolom LookUp (1-based) */
var JD_LU = { NO:1, NPK:2, NAMA:3, PK:5, KATEGORI:6, MULAI_PKSC:7, MULAI_UPI:8, KELUAR:9, LOKASI:10, AKTIF:11, JAM_KERJA:12, KODE_WORKPLAZE:13, KODE_DINAS:15, LIBUR:24 };
/* Kolom sheet utama (1-based): A=NPK, B=Nama, C.. = tanggal */
var JD_MAIN = { NPK:1, NAMA:2, FIRST_DATE:3 };

/* Memoize per-eksekusi (tiap request = eksekusi baru) */
var _JD_SS = null, _JD_TZ = null;
function jd_ss_(){ if(!_JD_SS) _JD_SS = SpreadsheetApp.openById(PD_SS_ID); return _JD_SS; }
function jd_tz_(){ if(!_JD_TZ) _JD_TZ = jd_ss_().getSpreadsheetTimeZone() || 'Asia/Jakarta'; return _JD_TZ; }

function jd_norm_(v){ return (v===null||v===undefined) ? '' : String(v).trim(); }
function jd_npk_(v){
  if(v===null||v===undefined) return '';
  if(typeof v==='number') return String(Math.round(v));
  return String(v).trim();
}
function jd_dateKey_(d){
  if(!(d instanceof Date) || isNaN(d.getTime())) return '';
  return Utilities.formatDate(d, jd_tz_(), 'yyyy-MM-dd');
}
function jd_dateDisp_(d){
  if(!(d instanceof Date) || isNaN(d.getTime())) return '';
  return Utilities.formatDate(d, jd_tz_(), 'd-MMM-yyyy');
}
/* Parse berbagai format tanggal -> Date (tengah hari, hindari geser TZ) */
function jd_parseDate_(s){
  if(s instanceof Date) return isNaN(s.getTime()) ? null : s;
  s = jd_norm_(s);
  if(!s) return null;
  var m = s.match(/^(\d{4})-(\d{1,2})-(\d{1,2})$/);
  if(m) return new Date(+m[1], +m[2]-1, +m[3], 12,0,0);
  var months = { jan:0,feb:1,mar:2,apr:3,may:4,mei:4,jun:5,jul:6,aug:7,agu:7,agt:7,sep:8,sept:8,oct:9,okt:9,nov:10,des:11,dec:11,
    januari:0,februari:1,maret:2,april:3,juni:5,juli:6,agustus:7,september:8,oktober:9,november:10,desember:11 };
  m = s.match(/^(\d{1,2})[\-\s]+([A-Za-z]+)[\-\s]+(\d{4})$/);
  if(m){ var mo = months[m[2].toLowerCase()]; if(mo!==undefined) return new Date(+m[3], mo, +m[1], 12,0,0); }
  m = s.match(/^(\d{1,2})[\/\.](\d{1,2})[\/\.](\d{2,4})$/);
  if(m){
    var a=+m[1], b=+m[2], y=+m[3]; if(y<100) y+=2000;
    var day, mon;
    if(a>12){ day=a; mon=b; } else if(b>12){ day=b; mon=a; } else { day=a; mon=b; }
    return new Date(y, mon-1, day, 12,0,0);
  }
  var d = new Date(s);
  return isNaN(d.getTime()) ? null : d;
}
function jd_dcell_(v, tz){ return (v instanceof Date) ? Utilities.formatDate(v, tz||jd_tz_(), 'd-MMM-yyyy') : jd_norm_(v); }
function jd_bool_(v){
  if(v===true) return true;
  var s = jd_norm_(v).toUpperCase();
  return s==='TRUE' || s==='1' || s==='YA' || s==='AKTIF';
}

/* Cari sheet utama untuk sebuah tahun. */
function jd_mainSheet_(tahun){
  var ss = jd_ss_();
  var byName = ss.getSheetByName('Daftar Dinas Perawat ' + tahun);
  if(byName) return byName;
  var sheets = ss.getSheets();
  for(var i=0;i<sheets.length;i++){
    var n = sheets[i].getName();
    if(/daftar dinas perawat/i.test(n) && n.indexOf(String(tahun))!==-1) return sheets[i];
  }
  for(var j=0;j<sheets.length;j++){
    var sh = sheets[j];
    if(sh.getLastColumn()<3 || sh.getLastRow()<1) continue;
    var head = sh.getRange(1,1,1,3).getValues()[0];
    if(jd_norm_(head[0]).toUpperCase()==='NPK' && head[2] instanceof Date && head[2].getFullYear()===Number(tahun)) return sh;
  }
  throw new Error('Sheet "Daftar Dinas Perawat ' + tahun + '" tidak ditemukan.');
}
/* Daftar tahun tersedia dari tab sheet. */
function jd_years_(){
  var years = [];
  var sheets = jd_ss_().getSheets();
  for(var i=0;i<sheets.length;i++){
    var m = sheets[i].getName().match(/daftar dinas perawat\s*(\d{4})/i);
    if(m && years.indexOf(m[1])===-1) years.push(m[1]);
  }
  years.sort();
  if(!years.length) years.push(String(new Date().getFullYear()));
  return years;
}
/* Baca LookUp L..X SEKALI: {codeList, map:O->M, jam:O->L, holidays} */
function jd_lookupBundle_(){
  var empty = { codeList:[], map:{}, jam:{}, holidays:[] };
  var sh = jd_ss_().getSheetByName(JD_LOOKUP_SHEET);
  if(!sh) return empty;
  var lastRow = sh.getLastRow();
  if(lastRow<2) return empty;
  var startCol = JD_LU.JAM_KERJA, endCol = JD_LU.LIBUR;
  var block = sh.getRange(2, startCol, lastRow-1, endCol-startCol+1).getValues();
  var lIdx = JD_LU.JAM_KERJA-startCol, mIdx = JD_LU.KODE_WORKPLAZE-startCol, oIdx = JD_LU.KODE_DINAS-startCol, xIdx = JD_LU.LIBUR-startCol;
  var codeList=[], seen={}, map={}, jam={}, holidays=[], hseen={};
  for(var i=0;i<block.length;i++){
    var o = jd_norm_(block[i][oIdx]);
    if(o!=='' && !seen[o]){
      seen[o]=1; codeList.push(o); map[o]=jd_norm_(block[i][mIdx]);
      var lv = block[i][lIdx];
      jam[o] = (lv===''||lv===null||lv===undefined) ? 0 : (Number(lv)||0);
    }
    var xv = block[i][xIdx];
    var d = xv instanceof Date ? xv : jd_parseDate_(xv);
    if(d && !isNaN(d.getTime())){ var k=jd_dateKey_(d); if(!hseen[k]){ hseen[k]=1; holidays.push(k); } }
  }
  return { codeList:codeList, map:map, jam:jam, holidays:holidays };
}
/* Matriks satu tahun: {tahun, headers:[{key,disp,col}], rows:[{npk,nama,codes[]}]} */
function jd_daftarDinas_(tahun){
  var sheet = jd_mainSheet_(tahun);
  var lastRow = sheet.getLastRow(), lastCol = sheet.getLastColumn();
  if(lastRow<1 || lastCol<JD_MAIN.FIRST_DATE) return { tahun:String(tahun), headers:[], rows:[] };
  var all = sheet.getRange(1,1,lastRow,lastCol).getValues();
  var headerRow = all[0], headers=[];
  for(var c=JD_MAIN.FIRST_DATE-1;c<lastCol;c++){
    var hv = headerRow[c];
    var d = hv instanceof Date ? hv : jd_parseDate_(hv);
    if(d && !isNaN(d.getTime())) headers.push({ key:jd_dateKey_(d), disp:jd_dateDisp_(d), col:c+1 });
  }
  var rows=[];
  for(var r=1;r<all.length;r++){
    var row = all[r];
    var npk = jd_npk_(row[JD_MAIN.NPK-1]);
    var nama = jd_norm_(row[JD_MAIN.NAMA-1]);
    if(!npk && !nama) continue;
    var arr=[];
    for(var i=0;i<headers.length;i++) arr.push(jd_norm_(row[headers[i].col-1]));
    rows.push({ npk:npk, nama:nama, codes:arr });
  }
  return { tahun:String(tahun), headers:headers, rows:rows };
}
/* Data staf (LookUp A..K) untuk filter/label. */
function jd_staff_(){
  var sh = jd_ss_().getSheetByName(JD_LOOKUP_SHEET);
  if(!sh) return [];
  var lastRow = sh.getLastRow();
  if(lastRow<2) return [];
  var vals = sh.getRange(2,1,lastRow-1,JD_LU.AKTIF).getValues();
  var tz = jd_tz_();
  var out=[];
  for(var i=0;i<vals.length;i++){
    var row = vals[i];
    var npk = jd_npk_(row[JD_LU.NPK-1]);
    var nama = jd_norm_(row[JD_LU.NAMA-1]);
    if(!npk && !nama) continue;
    out.push({ row:2+i, no:jd_norm_(row[JD_LU.NO-1]), npk:npk, nama:nama,
      pk:jd_norm_(row[JD_LU.PK-1]), kategori:jd_norm_(row[JD_LU.KATEGORI-1]),
      mulaiPksc:jd_dcell_(row[JD_LU.MULAI_PKSC-1],tz), mulaiUpi:jd_dcell_(row[JD_LU.MULAI_UPI-1],tz),
      keluar:jd_dcell_(row[JD_LU.KELUAR-1],tz), lokasi:jd_norm_(row[JD_LU.LOKASI-1]), aktif:jd_bool_(row[JD_LU.AKTIF-1]) });
  }
  return out;
}

/* ---- Fungsi yang dipanggil client (kembalikan JSON string) ---- */
/* Bundle dipanggil SEKALI saat load: LookUp + matriks tahun ini. */
function jdGetBundle(tahun){
  try{
    var years = jd_years_();
    var now = new Date();
    if(!tahun){ var curY=String(now.getFullYear()); tahun = (years.indexOf(curY)!==-1) ? curY : years[years.length-1]; }
    var dd = jd_daftarDinas_(tahun);
    var lk = jd_lookupBundle_();
    return JSON.stringify({ ok:true, tahun:String(tahun), years:years, today:jd_dateKey_(now),
      headers:dd.headers, rows:dd.rows, staff:jd_staff_(),
      codeList:lk.codeList, workplazeMap:lk.map, jamMap:lk.jam, holidays:lk.holidays });
  } catch(e){ return JSON.stringify({ ok:false, error:e.message }); }
}
/* Matriks satu tahun saja (ganti tahun / lintas-tahun) — RINGAN. */
function jdGetYear(tahun){
  try{
    var dd = jd_daftarDinas_(tahun);
    return JSON.stringify({ ok:true, tahun:dd.tahun, headers:dd.headers, rows:dd.rows });
  } catch(e){ return JSON.stringify({ ok:false, error:e.message }); }
}
