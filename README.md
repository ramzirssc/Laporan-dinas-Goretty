...
| `assets/logo_bunga_base64.txt` | Sumber logo topbar — logo bunga RS St. Carolus latar transparan (data URI PNG base64). File repo saja — **tidak** di-`push` clasp (ekstensi tak dikenal). Isinya di-*inline* ke `src` `<img class="topbar-logo">` di `index.html`. (File lama `assets/logo_carolus_base64.txt`, logo lengkap format WebP, masih ada di repo tapi sudah tak dipakai.) |
...
> **Logo topbar.** Logo bunga RS St. Carolus (latar transparan) tampil di ujung kiri topbar (`height:53px`), sebelum blok teks "RS St. Carolus / IPI — Goretty" (`<img class="topbar-logo">`). Sumbernya data URI PNG base64 yang di-*inline* langsung ke atribut `src` (bukan file yang diserve terpisah — HtmlService tak melayani gambar statis lewat URL). Master datanya di `assets/logo_bunga_base64.txt` (sumber asli: `logo bunga latar transparan.png`, folder lokal `LOGO`, diresize tinggi 160px untuk memperkecil payload base64); bila logo diganti, perbarui file itu lalu inline ulang ke `src`. Base64 aman di atribut `src` berkutip ganda (tak ada karakter kutip di dalamnya). Ukuran & warna topbar (§ tema di bawah) disamakan dengan proyek Damianus.
...
> **Layout mobile (≤767px): topbar 2-baris di Page1 + filter collapsible Page3/4.** Sebelumnya di HP, topbar besar + filter/header per-halaman menggerus nyaris seluruh layar sebelum konten kelihatan (Page1/3/4 mengunci konten ke `height:calc(100vh - var(--topbar-h) - Xpx)` yang di-scroll di dalam kotaknya sendiri — beda dari Page8/9 yang membiarkan `body` scroll biasa). Perbaikannya:
> - **Topbar.** Logo + `.tabs` dibungkus `.topbar-scroll` (`flex:1;overflow-x:auto`, scrollbar disembunyikan) sehingga tab tetap **satu baris yang bisa digeser** di mobile, terpisah dari `.topbar-info`. `.topbar-info` (tanggal/dokter jaga/jumlah pasien/tombol Refresh & wizard — hanya relevan di Page1, `body:not([data-page="p1"]) .topbar-info{display:none}` via `data-page` yang di-set `gotoPage()`) diberi `flex:1 1 100%` **khusus di dalam `@media(max-width:767px)`** — trik flexbox ini memaksanya selalu turun ke baris sendiri di bawah baris logo+tab (`.topbar-inner{flex-wrap:wrap}`), bukan berbagi baris dengan tab. **Awas:** kalau `.topbar-info` dibiarkan `flex-shrink:0` sebaris dengan tab (tanpa `flex-basis:100%`), di Page1 ia bisa memakan hampir seluruh lebar layar (2 tombol lebar) dan menyusutkan area scroll tab sampai beberapa piksel saja — nyaris tak terjangkau. `.topbar-scroll` juga butuh `flex:1` di aturan dasar (bukan cuma di media query mobile) supaya perilaku desktop lama (tab mengisi ruang tersisa, info rata kanan) tetap sama, karena `.tabs` sekarang bersarang satu level lebih dalam dari `.topbar-inner`. Struktur div `.topbar-scroll` **harus** menutup tepat setelah `.tabs` (sebelum `.topbar-info`) — kalau tertutup setelah `.topbar-info`, info ikut masuk area scroll dan bug di atas kembali muncul walau `flex-basis:100%` sudah benar.
> - **Filter collapsible (Page3/Page4).** Utility generik `.filt-toggle`/`.filt-collapsible` di `index.html` (default `display:none`/selalu tampil di luar breakpoint mobile, jadi desktop tak berubah): tombol `.filt-toggle` (judul + chevron `▾` yang berputar saat `.open`) membungkus panel filter (`.p3f` Page3, `.p4df` Page4) jadi `.filt-collapsible`, default tertutup di mobile. Toggle lewat `p3ToggleFilter()`/`p4dToggleFilter()` (`classList.toggle('open')` pada panel + tombol). Hasil pencarian (`.p3-cnt`/`#p4d-cnt`) **tetap di luar** panel supaya konteks ("N laporan ditemukan") terlihat walau filter tertutup. Page4 menambahkan `.filt-toggle` ke daftar selector `display:none!important` saat `@media print`; Page3 tak perlu karena tombolnya sudah ikut tersembunyi lewat `.p3-top{display:none!important}` yang membungkusnya.
> - Pola & alasan desain lengkap (termasuk perbandingan dengan proyek sister Damianus) ada di riwayat commit; tidak ada catatan kerja terpisah yang dipertahankan di repo.

> **Kelas CSS bersama (`.status-bar`, `.print-header`, `.noprint`).** Dulu tiap halaman mendefinisikan ulang CSS-nya sendiri untuk bar status di bawah filter (`.p3-cnt`/`.p4d-cnt`/`.p8-status`/`.p9-status`, byte-identik satu sama lain) dan untuk header cetak PDF tersembunyi (`.p3-print-header`/`.p4d-print-title`/`.p9-print-header`). Sekarang kedua pola itu diekstrak jadi kelas global di `index.html`, dan **Page3/Page4/Page8/Page9 semuanya sudah dimigrasikan**:
> - `.status-bar` — pill italic muted, radius+margin sendiri, dipakai berdiri sendiri (Page8, Page9).
>   Modifier **`.status-bar--joined`** — tanpa radius/margin, border atas/bawah dihapus, dipakai saat bar ini "menyatu" di antara card filter & area scroll (Page3, Page4).
> - `.print-header` — kotak `display:none` yang baru muncul via `@media print{display:block!important}` (aturan global, bukan per-halaman lagi), berisi `<h1>` judul + `<p id="…-print-sub">` subjudul yang diisi per halaman saat tombol Cetak diklik/data dirender.
>   Modifier **`.print-header--compact`** — judul & subjudul sedikit lebih kecil (13pt/9pt, dipakai Page4 sesuai ukuran aslinya; default 14pt/10pt dipakai Page3/Page9).
> - `.noprint{display:none!important}` dan `.topbar{display:none!important}` dan `body{background:#fff!important}` — semua sudah global di `@media print` (`index.html`), **jangan** didefinisikan ulang per halaman.
>
> Elemen yang disembunyikan saat print selain topbar/status-bar (mis. `.p3-top`, `#p4d-cnt`, `.p9-bar`) tetap didefinisikan **per halaman** di `@media print` masing-masing, karena struktur DOM tiap halaman beda. Kelas tabel (`.p3t`/`.p8-tbl`/`.p9-tbl`) **sengaja tidak** disatukan — alignment, warna header, cara strip baris, dan word-wrap-nya berbeda nyata sesuai bentuk data tiap halaman (teks bebas vs statistik vs numerik), bukan sekadar beda nama kelas.

> **Cache single-key: `cache.put` selalu dibungkus `try/catch`.** `CacheService` punya batas ukuran per key (~100KB); kalau `cache.put` gagal karena payload kelewat besar dan tidak dibungkus, exception-nya bisa menggagalkan **hasil yang sudah berhasil dihitung** (contoh nyata: `getDataPage1` dulu — `cache.put`-nya ada di dalam `try` besar yang sama dengan komputasi, jadi kegagalan cache.put jatuh ke `catch` luar dan klien menerima blob error, padahal datanya sudah beres dihitung). Semua fungsi cache single-key (`opsi`, `getDataPage1`, `getInitialData`, `getDaftarDinas`, `getStatistikPage8`, `getPivotBulananPage8`) sekarang konsisten: `try{cache.put(...);}catch(e){}` — kegagalan tulis cache diabaikan, hasil yang sudah dihitung tetap dikembalikan ke klien. **Tidak** ditarik jadi satu fungsi helper generik (`lbCacheGetOrCompute_` dsb) karena beberapa fungsi **sengaja** tidak menulis cache pada early-return tertentu (state kosong/error) — menyeragamkan lewat satu wrapper akan diam-diam mengubah perilaku itu.
...
- **Antibiotik** — 5 slot, `Antibiotik{1-5}Nama=...` / `Antibiotik{1-5}Freq=...` / `Antibiotik{1-5}Berat=...` / `Antibiotik{1-5}Per=...`. **Nama** = dropdown dari `Lookup!K2:K` (`opsiantibiotik()`, lazy, cache lewat `opsi()`). **Freq** dibakukan **hanya angka 1–10** (`<select>`, bukan teks bebas — tak bisa diisi teks lain) — makna "berapa kali per hari". **Berat** (dosis) **teks bebas** (mis. `100 mg`, `200 mg`, `1 gram`) — dosis anak-anak sulit dibakukan ke daftar tetap, jadi cuma jenis antibiotiknya yang dipilih dari daftar baku, dosis & frekuensi bebas/dibatasi manual. **Per** = interval pemberian dalam jam, `<select>` nilai tetap **hardcode** `24`/`36`/`48`/`72` (bukan dari sheet Lookup), default **`24`** — untuk antibiotik yang diberikan tak tiap hari (mis. 1×500mg tiap 48 jam), supaya Freq (berapa kali PER PEMBERIAN itu, biasanya `1`) tak disalahartikan shift berikutnya sebagai "berapa kali per hari". **Slot yang Nama-nya kosong TIDAK ditulis sama sekali** (`Antibiotik3Nama=` kosong tidak ditulis ke sheet) — supaya string kolom I tak membengkak percuma & parsing baliknya bersih. Freq/Berat/Per hanya bermakna kalau Nama terisi; parsing balik memprosesnya **atomik per slot** (nama+freq+berat+per sekaligus dari token slot yang sama) supaya tidak nyasar tertukar antar slot. **Backward-compatible**: laporan lama tanpa token `Per` (dibuat sebelum fitur ini ada) diparse sebagai `Per='24'` tanpa migrasi data apa pun. Tampilan ringkas (`p3AlatViewHtml` di Page3, `p4PpiRowsHtml` di Page4) menambahkan suffix `/Xj` (mis. `Ceftriaxone 1x 500mg /48j`) **hanya bila `Per` bukan default `24`**, supaya tampilan tetap bersih untuk mayoritas kasus dosis harian biasa. **Dikecualikan dari statistik/pivot** (`getStatistikPage8`/`getPivotBulananPage8`) secara otomatis, sama seperti Freq/Berat — token `Key=Value` tak pernah cocok terhadap `ALAT_LIST` yang dicocokkan sebagai token bulat, jadi tak perlu logika exclusion terpisah.
