...
> **Kelas CSS bersama (`.status-bar`, `.print-header`, `.noprint`).** Dulu tiap halaman mendefinisikan ulang CSS-nya sendiri untuk bar status di bawah filter (`.p3-cnt`/`.p4d-cnt`/`.p8-status`/`.p9-status`, byte-identik satu sama lain) dan untuk header cetak PDF tersembunyi (`.p3-print-header`/`.p4d-print-title`/`.p9-print-header`). Sejak refactor Page9, kedua pola itu diekstrak jadi kelas global di `index.html`: `.status-bar` (pill italic muted, radius+margin sendiri — dipakai berdiri sendiri seperti Page8/Page9) dan `.print-header` (kotak `display:none` yang baru muncul via `@media print{display:block!important}`, dipasang tombol `id="…-print-sub"` per halaman untuk teks subjudul cetak) — plus `.noprint{display:none!important}` (global, `@media print`) untuk menyembunyikan elemen apa pun (mis. tombol Cetak sendiri) saat mencetak. **Page9 sudah memakai kelas ini** (`page9.html`); **Page3/Page4/Page8 belum dimigrasikan** — mereka masih pakai kelas lokal `-cnt`/`-status`/`-print-header`/`-print-title` masing-masing (secara visual identik, cuma belum disatukan). Bila memigrasikan Page3/Page4, `.status-bar` versi mereka perlu varian "menyatu" (tanpa radius/margin, duduk rapat antara card filter & area scroll) — jangan pakai `.status-bar` polos begitu saja di sana tanpa modifier, nanti tampilannya berubah.
